...
_sdk_ping_cache: dict = {"result": False, "timestamp": 0.0}
_SDK_PING_TTL: float = 5.0  # seconds between liveness checks (matches M4L ping TTL)


def get_sdk_client() -> Optional[ExtensionsSDKClient]:
    """Return the Extensions SDK client if the bridge is reachable, else None.

    Uses a TTL-cached liveness check (5s) to avoid a blocking HTTP /health call
    on every tool invocation — mirrors the M4L bridge ping cache pattern.
    Returns None (not raises) so callers can fall back gracefully.
    """
    global _sdk_client, _sdk_ping_cache
    if _sdk_client is None:
        _sdk_client = ExtensionsSDKClient()

    now = _time.time()
    if (now - _sdk_ping_cache["timestamp"]) < _SDK_PING_TTL:
        return _sdk_client if _sdk_ping_cache["result"] else None

    available = _sdk_client.is_available()
    _sdk_ping_cache = {"result": available, "timestamp": now}
    return _sdk_client if available else None
