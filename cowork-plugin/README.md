...
- AbletonBridge installed and running:
  ...
  - MCP server cloned from [github.com/jon-wrennall/AbletonBridge](https://github.com/jon-wrennall/AbletonBridge)
...
The plugin connects to the AbletonBridge MCP server via `.mcp.json`. Before installing,
update the `--directory` path in `.mcp.json` to wherever you cloned this repo:

```json
"args": ["run", "--directory", "/your/path/to/AbletonBridge", "MCP_Server/server.py"]
```
